...
class FetchCreateInboundJob < ActiveJob::Base
  ...
  def get_inbound_user_data inbound_link
    begin
      ...
      PipecandyMailer.inbound_autofollow_error("Inbound - Get Inbound User Data In JOB exception errors", "#{e.to_s}").deliver_now
    rescue Exception => e
      puts "Error Message: #{e.to_s}"
      PipecandyMailer.inbound_autofollow_error("Inbound - Get Inbound User Data In JOB exception errors", "#{e.to_s}").deliver_now
    end  
  ...
  def iterate_search_page search_page
    begin
      ...
      puts "Error Message: #{e.to_s}"
      PipecandyMailer.inbound_autofollow_error("Inbound - Iterative Search Page In JOB exception errors", "#{e.to_s}").deliver_now
    rescue Exception => e
      puts "Error Message: #{e.to_s}"
      PipecandyMailer.inbound_autofollow_error("Inbound - Iterative Search Page In JOB exception errors", "#{e.to_s}").deliver_now
    end
